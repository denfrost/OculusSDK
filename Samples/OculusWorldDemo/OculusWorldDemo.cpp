    ...
    MenuIsTranslating(false)
{
    ...
    if (!EnvErr && EnvDefaultSceneRenderMode[0])
    {
        ...
        if (EnvRenderModeAsLong <= (unsigned long)Scene_PerfTest)
        {
        ...
        else
        {
            WriteLog("[OculusWorldDemoApp] Warning: Unable to set OWD_DEFAULT_SCENE_RENDER_MODE to %i. "
                            ...
                            EnvRenderModeAsLong, (unsigned long)Scene_PerfTest);
        }
    }
...
// Apparently one of the build targets in the build machine is non-private
    ...
    else
    {
        for (int layerNum = 0; layerNum < CamLayerCount; layerNum++)
        {
            ...
            for (int eye = 0; eye < 2; eye++)
            {
                ...
                EyeLayer[layerNum].EyeFov.RenderPose[eye]     = CamRenderPose[camNum]; // Where the eye was when we asked the SDK earlier.
    ...
    // as much by queue ahead logic.
    {
        ...
        if (MirrorTexture)
        {
            ...
            {
                 pRender->Blt(MirrorTexture, 0, 0, DefaultWindowWidth, DefaultWindowHeight);
            }
            else
            {
